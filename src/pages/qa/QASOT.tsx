...
export default function QASOT() {
  ...
  useEffect(() => {
    ...
    const items: SOTItem[] = [
      ...
      // Phase 3.3 baseline checks
      {
        item: 'Routing: Landing → /choose-role',
        claimed_in_sot: 'v2.3.5',
        status: state.onboarding.get_started_to === '/choose-role' ? 'PASS' : 'FAIL',
        notes: `Get Started routes to: ${state.onboarding.get_started_to}`,
      },
      {
        item: 'Admin v2 present (password-only)',
        claimed_in_sot: 'v2.3.5',
        status: state.admin.present && state.admin.google_disabled_in_admin ? 'PASS' : 'FAIL',
        notes: state.admin.present ? 'Admin system present' : 'Admin system missing',
      },
      {
        item: 'Admin allowlist env configured',
        claimed_in_sot: 'v2.3.5',
        status: state.admin.allowlist.primary_email_set && state.admin.allowlist.allowed_emails_set && state.admin.allowlist.allowed_domains_set ? 'PASS' : 'FAIL',
        notes: `Primary: ${state.admin.allowlist.primary_email_set}, Emails: ${state.admin.allowlist.allowed_emails_set}, Domains: ${state.admin.allowlist.allowed_domains_set}`,
      },
      {
        item: 'Admin sections (14 total)',
        claimed_in_sot: 'v2.3.5',
        status: Object.values(state.admin.sections_present).every(v => v) ? 'PASS' : 'FAIL',
        notes: `${Object.values(state.admin.sections_present).filter(v => v).length}/14 sections present`,
      },
      {
        item: 'OCR enabled & KYC table present',
        claimed_in_sot: 'v2.3.5',
        status: state.onboarding.ocr_enabled && state.onboarding.kyc_documents_table_present ? 'PASS' : 'FAIL',
        notes: `OCR: ${state.onboarding.ocr_enabled}, KYC table: ${state.onboarding.kyc_documents_table_present}`,
      },
      {
        item: 'Driver & Courier OCR wired',
        claimed_in_sot: 'v2.3.5',
        status: state.onboarding.driver_ocr_wired && state.onboarding.courier_ocr_wired ? 'PASS' : 'FAIL',
        notes: `Driver: ${state.onboarding.driver_ocr_wired}, Courier: ${state.onboarding.courier_ocr_wired}`,
      },
      {
        item: 'Maps provider configurable',
        claimed_in_sot: 'v2.3.5',
        status: state.maps.provider === 'stub' || state.maps.provider === 'google' ? 'PASS' : 'FAIL',
        notes: `Current provider: ${state.maps.provider}, Google key: ${state.maps.has_google_key}`,
      },
      {
        item: 'Realtime presence channel configured',
        claimed_in_sot: 'v2.3.5',
        status: state.realtime.driver_presence_channel_configured ? 'PASS' : 'UNKNOWN',
        notes: `Channel: ${state.realtime.driver_presence_channel_configured}, Markers: ${state.realtime.marker_icons_by_vehicle}`,
      },
      {
        item: 'Footer admin link present',
        claimed_in_sot: 'v2.3.5',
        status: state.branding.footer_admin_link ? 'PASS' : 'FAIL',
        notes: 'Admin link in landing footer',
      },
      {
        item: 'KYC storage bucket present',
        claimed_in_sot: 'v2.3.5',
        status: state.kyc.storage_bucket_present ? 'PASS' : 'FAIL',
        notes: 'kyc bucket for document images',
      },
    ];
